...
class FirstWordSM(sm.SM):
    # Your code here
    startState = 'beforeFirstWord'
    def getNextValues(self, state, inp):
        if state == 'beforeFirstWord':
            if inp == ' ' or inp == '\n':
                return (state, None)
            else:
                return ('firstWord', inp)
        elif state == 'firstWord':
            if inp != ' ' and inp != '\n':
                return (state, inp)
            elif inp == '\n':
                return ('beforeFirstWord', None)
            else:
                return ('afterFirstWord', None)
        else:
            if inp != '\n':
                return (state, None)
            else:
